...
Controls who can open the Space URL and whether this app uses its own Convex Auth account system:

| Value | Meaning |
| --- | --- |
| `"public"` | Anyone with the link can open the Space; no app-level sign-up/sign-in routes |
| `"space_auth"` | Anyone can open the Space URL, then sign in to this app's Convex Auth account system |
| `"viktor_auth"` | Viktor workspace members must pass Viktor access before the app loads |
...
How to get the signed-in user depends on `auth.mode` in `viktor.space.json`:

### `auth.mode: viktor_auth` — use `useViktorSession()`

`ViktorAuthAppRoutes` fetches `/__viktor_auth/me` **exactly once** at the route root and shares the server-validated session through context. Pages and components consume it with the `useViktorSession()` hook:

```tsx
import { useViktorSession } from "@/lib/viktor-spaces-access";

function ProfileBadge() {
  const { user, isLoading } = useViktorSession();
...
- `user` — `{ id, email?, name? }` or `null` when signed out
- `session` — the full session including the Space `resource` (audience, permissions), or `null`
- `isLoading` — `true` while the single `me` request is still in flight

**Never call `fetch("/__viktor_auth/me")` from pages or components.** The session is already fetched once by the route wrapper; extra fetches add a network roundtrip and a duplicated loading state on every mount.

### `auth.mode: space_auth` — use the Convex query
...
### Public access with product auth disabled

There is no platform-level user.
